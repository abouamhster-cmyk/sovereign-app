...
export default function LoveFireSportPage() {
  ...
  async function fetchStats() {
    try {
      const response = await fetch(`${API_URL}/api/lf/stats`);
  ...
  async function fetchGrants() {
    try {
      const response = await fetch(`${API_URL}/api/lf/grants`);
      const data = await response.json();
      if (data.success) setGrants(data.grants);
    } catch (error) {
      console.error("Erreur grants:", error);
    }
  }

  async function fetchContracts() {
    try {
      const response = await fetch(`${API_URL}/api/lf/contracts`);
      const data = await response.json();
      if (data.success) setContracts(data.contracts);
    } catch (error) {
      console.error("Erreur contracts:", error);
    }
  }
  ...
  async function createGrant() {
    ...
    const response = await fetch(`${API_URL}/api/lf/grant`, {
      method: "POST",
