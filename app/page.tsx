...
export default function DashboardPage() {
  ...
  async function fetchAllData() {
    ...
    if (dashboardData) {
      ...
      setPriorities(dashboardData.priorities || []);
      setUrgentTasks(dashboardData.urgentTasks || []);
      setActiveMissions(dashboardData.activeMissions || []);
    }
    ...
    try {
      ...
      if (data.success) {
        return {
          ...
          priorities: data.top_priorities?.map((p: any) => ({
            id: p.id, title: p.title, priority_reason: p.reason, score: p.score
          })) || [],
          urgentTasks: [...(data.overdue_tasks || []), ...(data.tasks_today || [])].slice(0, 5),
    ...
    datasets: [
      {
        ...
        data: weeklyProgress.map(w => {
          const dayIndex = weeklyProgress.findIndex(w2 => w2.day === w.day);
          const daySpending = recentSpending.filter(s => new Date(s.date).getDay() === dayIndex);
      ...
      },
      {
        ...
        data: weeklyProgress.map(w => {
          const dayIndex = weeklyProgress.findIndex(w2 => w2.day === w.day);
          const dayRevenue = recentRevenue.filter(r => new Date(r.date).getDay() === dayIndex);
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-24 px-4">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gradient-to-r from-gold-500/10 to-transparent border-l-4 border-gold-500 rounded-xl p-4">
          {isLoading ? (
            ...
            <div className="flex items-start gap-3"><Sparkles className="w-5 h-5 text-gold-500 mt-0.5 flex-shrink-0" /><p className="text-ivory text-sm leading-relaxed">{becksMessage || "Bonjour, je suis là."}</p></div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-4">
        {mood ? (
          ...
          <div><p className="text-sm text-gray-400 mb-3">😊 Comment te sens-tu aujourd'hui ?</p><div className="flex justify-between">{moodButtons.map((m) => (<button key={m.value} onClick={() => handleSaveMood(m.value)} className="flex flex-col items-center gap-1 p-2 rounded-lg hover:bg-white/10"><span className="text-xl">{m.emoji}</span><span className="text-[10px] text-gray-500">{m.label}</span></button>))}</div></div>
        )}
      </div>
