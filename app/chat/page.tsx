...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Afficher un toast avec l'intention détectée
      if (data.detected_intent && data.detected_intent !== "OTHER") {
        const intentLabels: Record<string, string> = {
          EMOTIONAL: "💬 Mode émotionnel",
          ...
          BUSINESS: "💰 Business",
          FARM: "🌾 Ferme",
          STRATEGIC: "👑 Mode stratégique"
        };
        toast.info(`🧠 ${intentLabels[data.detected_intent] || "Mode adapté"} activé`, { duration: 1500 });
      }
        ...
        <div className="flex items-center gap-2">
          ...
          {lastIntent && (
            <div className="ml-2 px-2 py-0.5 bg-gold-500/20 rounded-full text-[10px] text-gold-400">
              ...
              {lastIntent === "STRATEGIC" && "👑 Mode stratégique"}
            </div>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className={`p-2 rounded-full transition-all ${isSpeaking ? "bg-red-500/20 text-red-400" : "bg-gold-500/20 text-gold-500 hover:bg-gold-500/30"} disabled:opacity-50`}
          >
          ...
          <button 
            ...
            className="p-2 rounded-full bg-gold-500/20 text-gold-500 hover:bg-gold-500/30"
          >
            ...
            >
              ...
              <div className="p-4">
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-gold-500/20 hover:bg-gold-500/30 text-gold-500 py-2 rounded-xl text-sm"
                >
              ...
              <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2">
                {filteredConversations.map(conv => (
                  <div 
                    key={conv.id} 
                    className={`group p-3 rounded-xl cursor-pointer ${currentConversationId === conv.id ? "bg-gold-500/10 border border-gold-500/30" : "hover:bg-white/5"}`}
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100">
                        <button onClick={() => { setEditingTitleId(conv.id); setEditingTitle(conv.title); }}>
      ...
      <div className="shrink-0 border-t border-white/10 bg-midnight/90 backdrop-blur-lg p-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <input 
            ...
            placeholder={isRecording ? "🎤 Enregistrement vocal..." : "Pose une question, demande de l'aide, ou parle-moi..."} 
        ...
        <div className="mt-2 text-center">
          <span className="text-[10px] text-gray-600">
            💡 Becks s'adapte automatiquement à ce que tu demandes
          </span>
        </div>
      </div>
